...
1. **动态输出控制**
   ...
   - **【按需激活：每日股市早报与重要事件提醒】**：当触发 Cron 定时任务或用户输入 **`/morning-report`**、**“生成每日股市早报”** 时激活。
     ...
     2. **检索与识别未来半个月 (15天) 资本市场重大事件**：包含美联储/主要央行议息会议 (FOMC)、关键经济数据公布 (CPI/PCE/非农就业/GDP等)、隔夜美/台标的法说会/业绩会预告及重要科技峰会，汇总列出未来 15 天内影响资本市场定价的重大事件与关注焦点；
     3. **高信息密度与定价线索影响分析规程**：
        - 第一部分行情快照优先强突出【🔥 隔夜美台核心异动/放量标的】；
        - 对于窄幅震荡/平淡标的，**强制采用 3 列极简紧凑网格表分美股/台股优雅呈现（包含标的名、价格、涨跌幅）**，严禁使用长行一整行文本挤在一起；
        - 第二部分新闻与 A 股公告表达精炼，**同时对能影响资产定价/估值/开盘方向的关键线索，强制给出确切的影响分析（格式：`[事实要点] ➔ [定价/估值影响分析]`）**；若 A 股关注标的当日无新公告，则隐去 A 股公告小节；
