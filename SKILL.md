...
6. **动态真实出处附带与防伪造 URL 铁律**：静态表中的 URL 仅为检索模版。在生成任何研报、早报或分析时，凡引用了具体的 SEC 8-K 备案、Seeking Alpha 研报、台股 MOPS 营收或大行评级，**强制必须附带该次 `search_web` 实际抓取到的原网页真实 URL 链接 (Citations)**；引用的 URL 必须 100% 来自于搜索引擎的真实返回，严禁伪造凭空猜测的假 HTML 路径。如果摘要未显式暴露原链接，需注明 `[来源: 官方披露/公开检索]`。
